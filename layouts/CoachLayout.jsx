...
const SidebarContent = ({ onClose, ...rest }) => {
  return (
    <Box
      ...
      {LinkItems.map((link) => {
        return (
          <NavItem key={link.name} path={link.path} name={link.name} icon={link.icon}>
            {link.name}
          </NavItem>
        )
      })}
...
const NavItem = ({ icon, children, name, path, ...rest }) => {
  const Router = useRouter();
  const isActive = Router.pathname.includes(name.toLowerCase().replace(' ', '-'))
  return (
